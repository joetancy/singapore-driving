...
export function nearestPoint(x,z,a,b){const dx=b[0]-a[0],dz=b[1]-a[1],den=dx*dx+dz*dz;const t=den?clamp(((x-a[0])*dx+(z-a[1])*dz)/den,0,1):0;const px=a[0]+dx*t,pz=a[1]+dz*t;return {x:px,z:pz,d:Math.hypot(x-px,z-pz),t};}
export function inRing(x,z,ring){let hit=false;for(let i=0,j=ring.length-1;i<ring.length;j=i++){const a=ring[i],b=ring[j];if((a[1]>z)!==(b[1]>z)&&x<(b[0]-a[0])*(z-a[1])/(b[1]-a[1])+a[0])hit=!hit;}return hit;}
export function inPolygon(x,z,rings){return inRing(x,z,rings[0])&&!rings.slice(1).some(r=>inRing(x,z,r));}
export function touchesPolygon(x,z,rings,radius=1.15){if(inPolygon(x,z,rings))return true;return rings.some(r=>r.some((a,i)=>nearestPoint(x,z,a,r[(i+1)%r.length]).d<radius));}
export function stepCar(state,input,dt,onRoad=true){
 const throttle=input.forward?1:0,brake=input.back?1:0,handbrake=!!input.handbrake;
 let accel=0;
 if(throttle)accel=state.speed<-.2?12:7.4;
 ...
 const angle=state.steer*(.53/(1+Math.abs(state.speed)*.058));
